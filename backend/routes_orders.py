...
# A grocery order has no legitimate reason to exceed these, and without a cap
# one request could reserve an item's entire stock.
MAX_ITEM_QUANTITY = 50
MAX_ITEMS_PER_ORDER = 100

async def _owned_order(order_id: str, user: dict) -> dict:
    """Fetch an order only if it belongs to the caller.

    Order ids are sequential (ORD000001...), so a non-owned order returns 404
    rather than 403 — confirming that an id exists is itself a disclosure.
    """
...
async def _next_order_id() -> str:
    """Atomically allocate the next order id.

    This previously read the highest existing id and added one. Two checkouts
    landing together resolved to the same number, and order_id carries a
    unique index — so the second insert raised a duplicate-key error and the
    customer got a 500 with no order. Allocation is now one atomic operation.
    """
    ...
    if doc is None:
        # First allocation: seed from the highest id already stored, so we do
        # not collide with orders placed before this counter existed.
        last = await orders_collection.find_one(
            {"order_id": {"$regex": "^ORD[0-9]+$"}}, sort=[("order_id", -1)]
        )
        start = int(last["order_id"][3:]) if last else 0
        await counters_collection.update_one(
            {"_id": "order_id"}, {"$setOnInsert": {"seq": start}}, upsert=True
        )
        doc = await counters_collection.find_one_and_update(
            {"_id": "order_id"}, {"$inc": {"seq": 1}},
            return_document=ReturnDocument.AFTER,
        )
    return f"ORD{doc['seq']:06d}"
...
async def _reserve_stock(items: list) -> list:
    """Decrement stock for every item, or take none at all.

    The check and the decrement are a single update, so two customers racing
    for the last unit cannot both succeed. If a later item is short, whatever
    was already taken is handed back rather than left reserved.
    """
    reserved = []
    for item in items:
        ...
        if result.modified_count != 1:
            ...
            raise HTTPException(
                status_code=409,
                detail=f"Not enough stock for {item.get('name') or 'an item'}",
            )
...
async def create_order(
    ...
    # Take stock before creating the order so two customers cannot both buy
    # the last unit. Released again if anything below fails.
    reserved = await _reserve_stock(items)
...
async def cancel_order(order_id: str, reason: str = Body("Customer requested", embed=True), user: dict = Depends(get_current_user)):
...
@router.get("/orders/{order_id}/invoice-link")
async def invoice_link(order_id: str, user: dict = Depends(get_current_user)):
    """Mint a short-lived link to this order's invoice.

    The invoice opens in the system browser, which cannot carry an
    Authorization header, so access is granted instead by a signed token
    scoped to this one order and valid for five minutes.
    """
    await _owned_order(order_id, user)
    return {"url": f"/orders/{order_id}/invoice?t={create_invoice_token(order_id, user['id'])}"}


@router.get("/orders/{order_id}/invoice")
async def download_invoice(order_id: str, t: str = Query("")):
    # Invoices carry the customer's name, phone and full delivery address, and
    # order ids are sequential, so this cannot be left open.
    if not t:
        raise HTTPException(status_code=401, detail="Invoice link required")
    payload = decode_token(t)
    if payload.get("scope") != "invoice" or payload.get("order_id") != order_id:
        raise HTTPException(status_code=403, detail="Link is not valid for this order")

    order = await orders_collection.find_one({"order_id": order_id})
    if not order or payload.get("sub") not in (order.get("user_id"), order.get("customer_id")):
        raise HTTPException(status_code=404, detail="Order not found")
